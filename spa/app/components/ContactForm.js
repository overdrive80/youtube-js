export function ContactForm() {
  const d = document,
    ...
    $styles = d.getElementById("dinamic-styles");
...
/* Función privada del componente */
function validationsForm($form) {
    ...
    if (e.target.matches("[required]")) {
      ...
      if (pattern && input.value !== "") {
        ...
        return !regex.exec(input.value)
          ? $form.querySelector(`#${input.name}`).classList.add("is-active")
          : $form.querySelector(`#${input.name}`).classList.remove("is-active");
      }
  ...
  $form.addEventListener("submit", (e) => {
    ...
    const $loader = $form.querySelector(".contact-form-loader"),
      $response = $form.querySelector(".contact-form-response");

    // Capturamos el valor del input de email
